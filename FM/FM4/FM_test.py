...
python FM_test.py --dataset ml-tag --epoch 100 --pretrain -1 --batch_size 4096 --hidden_factor 256 --lr 0.01 --keep 0.7
...
import argparse
from FM.FM4 import LoadData as DATA
...
class FM(BaseEstimator, TransformerMixin):
    ...
    def partial_fit(self, data):  # fit a batch
        print("type(data['X'])", type(data['X']), "type(data['Y']):", type(data['Y']))
...
def make_save_file(dataset, hidden_factor, mla):
    pretrain_path = '../pretrain/fm_%s_%d' %(dataset, hidden_factor)
    if mla:
        pretrain_path += '_mla'
    if not os.path.exists(pretrain_path):
        os.makedirs(pretrain_path)
    save_file = pretrain_path+'/%s_%d' %(dataset, hidden_factor)
    return save_file

def train(path, dataset, verbose, hidden_factor, epoch, batch_size, lr, lamda, keep, optimizer, batch_norm, pretrain, mla):
    # Data loading
    data = DATA.LoadData(path, dataset)
    if verbose > 0:
        print("FM: dataset=%s, factors=%d, #epoch=%d, batch=%d, lr=%.4f, lambda=%.1e, keep=%.2f, optimizer=%s, batch_norm=%d"
              %(dataset, hidden_factor, epoch, batch_size, lr, lamda, keep, optimizer, batch_norm))

    print("type(data.Train_data):", type(data.Train_data), "type(data.Validation_data):", type(data.Validation_data), "type(data.Test_data)", type(data.Test_data))
    # Training
    t1 = time()
    model = FM(data.features_M, pretrain, make_save_file(dataset, hidden_factor, mla), hidden_factor, epoch, batch_size, lr, lamda, keep, optimizer, batch_norm, verbose, mla)
...
if __name__ == '__main__':
    ...
    # --dataset ml-tag --epoch 100 --pretrain -1 --batch_size 4096 --hidden_factor 256 --lr 0.01 --keep 0.7
    ...
    pretrain = -1
    mla = 0
    train(path, dataset, verbose, hidden_factor, epoch, batch_size, lr, lamda, keep, optimizer, batch_norm, pretrain, mla)
